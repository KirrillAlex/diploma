      ...
      $required = ['email', 'password', 'name', 'surname', 'phone'];
        ...
        foreach ($required as $check) {
            if (empty($_POST[$check])){
              $errors[$check] = 'Заполните поле это поле корректно!';
            }
            if ($check == 'email') {
        ...
        if (empty($errors)) {
            ...
            if (mysqli_num_rows($checked) > 0) {
                ...
                $new_user_query = "INSERT INTO users (firstName, lastName, phoneNumber, password, registrationDate, email) VALUES('$firstNname', '$lastName', '$phoneNumber',
                '$password', '$now', '$email')";
  ...
  <div class="page-wrapper">
    <div class="container container--with-sidebar">
      <header class="main-header">
        <a href="index.php">
          <img src="img/allRight.png" width="130" height="130" alt="Логитип Дела в порядке">
        </a>
      </header>
            ...
            <div class="form__row">
              ...
              <?php 
                ...
                if (!empty($errors['name'])) {
                 ...
                 echo '<input class="form__input' . $error_class . '" type="text" name="name" id="name" value="" placeholder="Имя">';
              ?>
            </div>

            <div class="form__row">
              <label class="form__label" for="surname">Фамилия <sup>*</sup></label>
              <?php 
                $error_class = '';
                if (!empty($errors['surname'])) {
                    $error_class = ' form__input--error';  
                    echo '<p class="form__message">' . $errors['surname'] . '</p>';
                }
                 echo '<input class="form__input' . $error_class . '" type="text" name="surname" id="surname" value="" placeholder="Фамилия">';
              ?>
            </div>

            <div class="form__row">
              <label class="form__label" for="name">Номер телефона<sup>*</sup></label>
              <?php 
                $error_class = '';
                if (!empty($errors['phone'])) {
                    $error_class = ' form__input--error';  
                    echo '<p class="form__message">' . $errors['phone'] . '</p>';
                }
                 echo '<input class="form__input' . $error_class . '" type="tel" pattern="[0-9]{11}" name="phone" id="phone" value="" placeholder="Номер">';
              ?>
            </div>
